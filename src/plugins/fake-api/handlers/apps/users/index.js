...
export const handlerAppsUsers = [
  ...
  const data =()=>{
    const updateData = (data) => (data1 = data);
  ...
  data1=data()
    // filter users
    let filteredUsers = data1.filter(user => ((user.fullName.toLowerCase().includes(queryLower) || user.email.toLowerCase().includes(queryLower)) && user.role === (role || user.role) && user.currentPlan === (plan || user.currentPlan) && user.status === (status || user.status))).reverse()
    ...
    const totalUsers = data1.length

    // total pages
    const totalPages = Math.ceil(totalUsers / itemsPerPageLocal)
    
    return HttpResponse.json({
      users: paginateArray(data1, itemsPerPageLocal, pageLocal),
      totalPages,
      totalUsers,
