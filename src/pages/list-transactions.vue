<template>
  <div>
    <div class="app-user-search-filter d-flex align-center flex-wrap gap-4 mb-2">
      ...
      <TransactionDialogForm @userData="addNewClient" />
    </div>
...
import TransactionDialogForm from "@/views/Reutilisable/TransactionDialogForm .vue";
const rowData = ref([]);
...
const fetchData = () => {
  return fetch("http://51.178.42.116:8089/api/transactions")
    .then(resp => {
      if (!resp.ok) throw new Error('Network response was not ok');
      return resp.json();
    })
    ...
    ({
      ...transaction,
      dateTransaction: formatTimestamp(transaction.dateTransaction),
      carte: {
        ...transaction.carte,
        dateCreation: formatTimestamp(transaction.carte.dateCreation),
        dateExpiration: formatTimestamp(transaction.carte.dateExpiration),
        forfaitExpiration: formatTimestamp(transaction.carte.forfaitExpiration)
      }
    })
