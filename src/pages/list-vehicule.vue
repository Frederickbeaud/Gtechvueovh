<template>
  <div>
    <div class="app-user-search-filter d-flex align-center flex-wrap gap-4 mb-2">
      ...
      <BusDialogForm @userData="addNewClient" />
    </div>
    ...
    <ModalVehicule :rowId="rowId" />

  </div>
</template>

<script setup>
import BusDialogForm from "@/views/Reutilisable/BusDialogForm.vue";
import ButtonComp from "@/views/Reutilisable/ButtonComp.vue";
import ModalVehicule from "@/views/Reutilisable/ModalVehicule.vue";
import {
  AgGridVue
} from "ag-grid-vue3";
...
{
  headerName: 'Derniere Trajet associé',
  field: "trajet"
},
{
  headerName: 'Dernier chauffeur',
  field: "chauffeur"
},
{
  headerName: 'Terminal',
...
const fetchData = () => {
  return fetch("http://51.178.42.116:8089/api/vehicules")
    .then(resp => {
      if (!resp.ok) throw new Error('Network response was not ok');
      return resp.json();
    })
    .then(data => data.map(bus =>
...
const addNewClient = async userData => {
  ...
  await fetch('http://51.178.42.116:8089/api/vehicules/create', {
    method: 'POST',
    headers: {
